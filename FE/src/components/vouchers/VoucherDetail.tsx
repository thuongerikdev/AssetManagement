...
<div className="overflow-x-auto">
          {/* Tính toán tổng Phát sinh Nợ và Phát sinh Có */}
          {(() => {
            const tongNo = entries.reduce((acc, entry) => acc + (entry.taiKhoanNo ? entry.soTien : 0), 0);
            const tongCo = entries.reduce((acc, entry) => acc + (entry.taiKhoanCo ? entry.soTien : 0), 0);
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-700 uppercase">Phát sinh Nợ</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-700 uppercase">Phát sinh Có</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {entries.length === 0 ? (
                    <tr><td colSpan={7} className="px-6 py-4 text-center text-gray-500">Chưa có chi tiết định khoản</td></tr>
                  ) : (
                    entries.map((entry, index) => (
                      <tr key={entry.id || index} className="hover:bg-gray-50">
                        ...
                        {/* Cột Phát sinh Nợ: Chỉ hiện tiền nếu có TK Nợ */}
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-gray-900">
                          {entry.taiKhoanNo ? formatCurrency(entry.soTien) : '-'}
                        </td>
                        
                        {/* Cột Phát sinh Có: Chỉ hiện tiền nếu có TK Có */}
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-gray-900">
                          {entry.taiKhoanCo ? formatCurrency(entry.soTien) : '-'}
                        </td>
                      </tr>
                    ))
                  )}
                ...
                <tfoot className="bg-gray-50 border-t-2 border-gray-300">
                  <tr>
                    <td colSpan={5} className="px-6 py-4 text-right font-semibold text-gray-900">Tổng phát sinh:</td>
                    <td className="px-6 py-4 text-right font-bold text-blue-600">
                      {formatCurrency(tongNo)}
                    </td>
                    <td className="px-6 py-4 text-right font-bold text-blue-600">
